...
const INITIAL_STEPS: WorkflowStep[] = [
  {
    id: "step-1",
    name: "NeuroMetric Stream Anomaly Trigger",
    type: "trigger",
    provider: "NeuroLabs Realtime Stream",
    icon: "Brain",
    color: "from-cyan-500 to-blue-600",
    badge: "Realtime Trigger",
    summary: "Listens for EEG biometric gamma spike > 85Hz on Active Channels",
    config: {
      event: "eeg.anomaly.gamma_spike",
      endpoint: "wss://stream.neurolabs.io/v2/telemetry",
      headers: "Authorization: Bearer neu_live_sk99214a",
      bodyTemplate: '{"channel": "frontal_cortex", "threshold": 0.85}',
    },
    sampleOutput: {
      event: "eeg.anomaly.gamma_spike",
      subjectId: "sub_884920",
      deviceSerial: "NL-HELMET-X4",
      confidence: 0.942,
      metrics: {
        gammaPower: 92.4,
        thetaRatio: 1.34,
        stressIndex: 0.89,
        timestamp: "2026-08-15T13:45:10.201Z",
      },
    },
    status: "idle",
  },
  {
    id: "step-2",
    name: "NeuroCore GPT-4o Cognitive Triage",
    type: "ai",
    provider: "OpenAI GPT-4o & NeuroLabs Engine",
    icon: "Sparkles",
    color: "from-purple-500 to-indigo-600",
    badge: "AI Inference",
    summary: "Synthesizes EEG anomaly against baseline profile & determines severity",
    config: {
      model: "gpt-4o-2024-08-06",
      prompt:
        "Analyze telemetry anomaly for subject {{step1.subjectId}} with stress index {{step1.metrics.stressIndex}}. Classify risk tier (CRITICAL, ELEVATED, NOMINAL) and generate mitigation recommendation.",
      temperature: 0.2,
      retries: 3,
    },
    sampleOutput: {
      classification: "CRITICAL_FATIGUE_OVERLOAD",
      severityScore: 0.91,
      requiresIntervention: true,
      executiveSummary:
        "Acute cognitive saturation detected in prefrontal lobe. Recommended action: 15-min sensory recalibration protocol.",
      tokenCostUsd: 0.0031,
      latency: "184ms",
    },
    status: "idle",
  ...
  {
    id: "step-3",
    name: "Condition: Severity > 0.80 & Requires Action",
    type: "condition",
    provider: "Logic Router",
    icon: "GitBranch",
    color: "from-amber-500 to-orange-600",
    badge: "Router / Branch",
    summary: "Evaluates `{{step2.severityScore}} >= 0.80` to route high-priority escalation",
    config: {
      condition: "step2.severityScore >= 0.80 && step2.requiresIntervention === true",
    },
    sampleOutput: {
      matched: true,
      branchSelected: "Branch A: Critical Priority Escalation",
      evaluatedAt: "2026-08-15T13:45:10.450Z",
    },
    status: "idle",
  },
  {
    id: "step-4",
    name: "Dispatch Slack Medical / Ops Alert",
    type: "action",
    provider: "Slack Enterprise",
    icon: "MessageSquare",
    color: "from-emerald-500 to-teal-600",
    badge: "Notification",
    summary: "Pings channel #neurolabs-critical-ops with interactive action buttons",
    config: {
      channel: "#neurolabs-critical-ops",
      recipient: "channel_ops_oncall",
      bodyTemplate:
        "🚨 *CRITICAL BIOMETRIC ALERT*\nSubject: `{{step1.subjectId}}`\nSeverity: `{{step2.severityScore}}`\nStatus: *Intervention Required*",
    },
    sampleOutput: {
      messageId: "msg_slk_892831",
      channel: "C08472910",
      deliveredAt: "2026-08-15T13:45:10.680Z",
      acknowledgedBy: null,
    },
    status: "idle",
  },
  {
    id: "step-5",
    name: "Append Incident to Supabase Postgres ERP",
    type: "action",
    provider: "Supabase Database",
    icon: "Database",
    color: "from-blue-500 to-cyan-600",
    badge: "Database Write",
    summary: "Inserts audit log row into `neuro_anomaly_incidents` table",
    config: {
      table: "neuro_anomaly_incidents",
      action: "INSERT",
      bodyTemplate:
        '{"subject_id": "{{step1.subjectId}}", "severity": {{step2.severityScore}}, "status": "OPEN", "raw_payload": {{step1.metrics}}}',
    },
    sampleOutput: {
      insertedRows: 1,
      recordId: "rec_eeg_9941a8",
...
const MOCK_WORKFLOWS: WorkflowItem[] = [
  {
    id: "wf-1",
    name: "Realtime EEG Biometric Triage & Alert Router",
    description:
      "Detects gamma telemetry anomalies in real-time, runs GPT-4o cognitive analysis, and dispatches on-call alerts.",
    category: "Neuro & AI",
    isActive: true,
    version: "v2.4",
    lastRun: "2 mins ago",
    ...
    successRate: 99.94,
    steps: INITIAL_STEPS,
    updatedAt: "2026-08-15 13:20",
  },
  {
    id: "wf-2",
    name: "Stripe VIP Customer AI Onboarding & Slack Sync",
    description:
      "When a new Enterprise tier invoice is paid, provisions database tenant and notifies customer success in Slack.",
    category: "CRM & Sales",
    isActive: true,
    version: "v1.8",
    lastRun: "18 mins ago",
    totalRuns: 1420,
    successRate: 100,
    steps: [INITIAL_STEPS[0], INITIAL_STEPS[3]],
    updatedAt: "2026-08-14 09:12",
  },
  {
    id: "wf-3",
...
export default function AutomationsPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#070B13] text-slate-100 flex flex-col antialiased selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        ...
        <div className="absolute bottom-[-10%] left-[35%] w-[600px] h-[600px] bg-blue-600/10 rounded-full blur-[180px]" />
      ...
      <header className="relative z-20 border-b border-slate-800/80 bg-slate-950/70 backdrop-blur-2xl px-4 lg:px-8 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 shadow-xl shadow-black/20">
            ...
            <div className="flex items-center gap-3 mt-0.5">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="text-base lg:text-lg font-bold text-white tracking-tight flex items-center gap-2 hover:text-cyan-300 transition-colors group">
                ...
                <DropdownMenuContent className="w-80 bg-slate-900 border-slate-800 text-slate-200 shadow-2xl backdrop-blur-xl">
                        ...
                        <Badge
                          ...
                          className={
                            wf.isActive
                              ...
                              : "bg-slate-800 text-slate-400 text-[9px]"
                          }
                        >
              ...
              <Badge
                variant="outline"
                className="hidden sm:inline-flex bg-slate-900 border-slate-700 text-slate-400 text-[10px] font-mono"
              >
        ...
        <div className="flex items-center gap-3 justify-end flex-wrap">
          ...
          <Button
            ...
            size="sm"
            className="bg-slate-900 hover:bg-slate-800 text-cyan-400 border border-cyan-800/60 shadow-lg shadow-cyan-950/40 text-xs font-semibold gap-1.5 transition-all hover:border-cyan-500"
          >
            {isTestingRun ? (
              <>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 bg-slate-900 border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800"
                >
                  {copiedCode ? (
          ...
          <Button
            ...
            size="sm"
            className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white shadow-lg shadow-cyan-500/25 text-xs font-semibold gap-1.5"
          >
      ...
      <main className="relative z-10 flex-1 flex flex-col overflow-hidden">
        ...
        {activeTab === "builder" && (
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="bg-slate-950/40 border-b border-slate-800/60 px-6 py-2.5 flex items-center justify-between text-xs text-slate-400 backdrop-blur-md">
              <div className="flex items-center gap-4">
                ...
                <Separator orientation="vertical" className="h-4 bg-slate-800 hidden sm:block" />
              ...
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className="bg-slate-900 border-slate-800 text-slate-300 text-[11px] gap-1.5"
                >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-8 lg:py-12 custom-scrollbar">
              <div className="max-w-3xl mx-auto flex flex-col items-center">
                    ...
                    <React.Fragment key={step.id}>
                      ...
                      <div
                        ...
                        className={`group relative w-full rounded-2xl transition-all duration-300 cursor-pointer ${
                          isCurrentSimulated
                            ? "ring-2 ring-cyan-400 shadow-2xl shadow-cyan-500/40 scale-[1.02]"
                            : isSimulatedSuccess
                            ? "ring-1 ring-emerald-500/80 shadow-lg shadow-emerald-500/20"
                            : "hover:scale-[1.01] hover:shadow-2xl hover:shadow-cyan-950/30"
                        }`}
                      >
                        ...
                        <div className="relative rounded-2xl bg-[#0C121F]/90 backdrop-blur-xl border border-slate-800/90 p-5 lg:p-6 text-slate-100 shadow-xl overflow-hidden">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3.5">
                              ...
                              >
                                <div className="h-full w-full bg-slate-950 rounded-[11px] flex items-center justify-center text-white">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="bg-slate-900/80 border-slate-700/60 text-slate-300 text-[10px] px-1.5 py-0"
                                  >
                                    {step.badge}
                                  </Badge>
                                </div>
                                <h3 className="text-base font-bold text-white tracking-tight mt-0.5 group-hover:text-cyan-300 transition-colors">
                                  {step.name}
                                </h3>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              {step.status === "running" || isCurrentSimulated ? (
                                ...
                                <Badge
                                  variant="secondary"
                                  className="bg-slate-900/80 text-slate-400 border border-slate-800 text-[11px]"
                                >
                                  Ready
                                </Badge>
                              )}
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent className="bg-slate-900 border-slate-800 text-slate-200 shadow-2xl">
                                  <DropdownMenuItem
                                    onClick={() => handleOpenInspector(step)}
                          ...
                          <div className="mt-3 text-xs text-slate-300 leading-relaxed bg-slate-950/40 rounded-xl p-3 border border-slate-800/60 font-mono">
                        ...
                        <div className="relative my-2 flex flex-col items-center justify-center group/connector">
                          ...
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                            <button
                              ...
                              className="h-7 px-3 rounded-full bg-slate-900 border border-slate-700 text-slate-300 hover:text-white hover:border-cyan-400 hover:bg-slate-800 hover:shadow-lg hover:shadow-cyan-500/20 text-[11px] font-semibold flex items-center gap-1.5 transition-all transform hover:scale-105"
                            >
                ...
                <div className="mt-6 flex flex-col items-center">
                  ...
                  <div className="px-4 py-2 rounded-full bg-slate-900/90 border border-slate-800 text-xs font-semibold text-slate-400 flex items-center gap-2 shadow-lg">
                  ...
                  <Button
                    onClick={() => handleOpenAddStep()}
                    variant="outline"
                    className="mt-6 bg-slate-900/80 border-dashed border-slate-700 hover:border-cyan-500 hover:text-cyan-400 text-slate-300 gap-2 text-xs font-semibold py-5 px-6 rounded-2xl shadow-xl transition-all"
                  >
        ...
        {activeTab === "workflows" && (
          <div className="flex-1 overflow-y-auto px-4 lg:px-8 py-8 custom-scrollbar">
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-4 lg:p-6 rounded-2xl border border-slate-800 backdrop-blur-xl">
                <div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="relative w-64">
                    ...
                    <Input
                      ...
                      className="pl-8 bg-slate-950 border-slate-800 text-xs text-slate-200 placeholder:text-slate-500 h-9"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {filteredWorkflows.map((wf) => (
                  <Card
                    key={wf.id}
                    className="bg-[#0C121F]/80 border-slate-800/80 hover:border-slate-700/90 transition-all duration-300 rounded-2xl shadow-xl backdrop-blur-xl hover:shadow-2xl hover:shadow-cyan-950/20 flex flex-col justify-between group"
                  >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <Badge
                                variant="outline"
                                className="bg-slate-900 border-slate-700 text-slate-300 text-[10px]"
                              >
                                {wf.category}
                              </Badge>
                              <Badge
                                variant="outline"
                                className="bg-slate-900 border-slate-800 text-slate-500 text-[10px] font-mono"
                              >
                                {wf.version}
                              </Badge>
                            </div>
                            <CardTitle className="text-base font-bold text-white mt-1 group-hover:text-cyan-300 transition-colors">
                              {wf.name}
                            </CardTitle>
                          </div>
                        </div>
                      ...
                      <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800/60 space-y-2">
                        ...
                        <div className="flex items-center gap-1.5 overflow-x-auto py-1">
                          {wf.steps.map((step, idx) => (
                            <React.Fragment key={step.id}>
                              <div className="flex items-center gap-1 px-2 py-1 rounded-md bg-slate-900 border border-slate-800 text-[10px] font-medium text-slate-300 whitespace-nowrap">
                    ...
                    <CardFooter className="pt-3 border-t border-slate-800/60 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          size="sm"
                          className="bg-slate-900 hover:bg-slate-800 text-cyan-400 border border-cyan-800/60 text-xs font-semibold gap-1.5"
                        >
          ...
          <div className="flex-1 overflow-y-auto px-4 lg:px-8 py-8 custom-scrollbar">
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="bg-slate-900/70 border-slate-800 p-5 rounded-2xl shadow-xl backdrop-blur-xl">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase text-slate-400">Total Runs (24h)</span>
                    <Activity className="h-4 w-4 text-cyan-400" />
                  </div>
                  <div className="text-2xl font-bold font-mono text-white mt-2">24,819</div>
              ...
              <Card className="bg-[#0C121F]/80 border-slate-800 rounded-2xl shadow-2xl backdrop-blur-xl overflow-hidden">
                <CardHeader className="border-b border-slate-800 px-6 py-4 flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-base font-bold text-white">Live Execution Stream</CardTitle>
                    <CardDescription className="text-xs text-slate-400">
                      Real-time chronological events processed through the automation engine.
                    </CardDescription>
                  </div>
                  <Badge variant="outline" className="bg-slate-950 border-slate-800 text-cyan-400 text-xs font-mono">
                ...
                <div className="divide-y divide-slate-800/60">
                  {MOCK_LOGS.map((log) => (
                    <div
                      key={log.id}
                      className="p-4 lg:px-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-900/50 transition-colors"
                    >
                      <div className="flex items-start gap-3.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold text-white">{log.id}</span>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <Badge
                              variant="outline"
                              className="bg-slate-950 border-slate-800 text-slate-400 text-[10px] font-mono"
                            >
                      ...
                      <div className="flex items-center gap-6 justify-between md:justify-end">
                        <div className="text-right font-mono text-xs">
                          <div className="font-bold text-slate-200">{log.durationMs}ms</div>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="bg-slate-900 border-slate-800 text-slate-200 max-w-lg">
                            ...
                            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-cyan-300 overflow-x-auto max-h-72 custom-scrollbar">
      ...
      <Sheet open={isInspectorOpen} onOpenChange={setIsInspectorOpen}>
        <SheetContent
          side="right"
          className="w-full sm:max-w-xl bg-slate-950/95 border-l border-slate-800 text-slate-100 backdrop-blur-2xl p-0 flex flex-col shadow-2xl z-50 overflow-hidden"
        >
          {selectedNode && (
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant="outline"
                        className="bg-slate-950 border-slate-800 text-slate-300 text-[10px]"
                      >
                        {selectedNode.badge}
                      </Badge>
                    </div>
                    <SheetTitle className="text-lg font-bold text-white mt-0.5">
                      {selectedNode.name}
                    </SheetTitle>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                <Tabs defaultValue="params" className="w-full">
                  <TabsList className="grid grid-cols-3 bg-slate-900 border border-slate-800 p-1 mb-6 rounded-xl">
                      ...
                      <>
                        <div className="space-y-1.5">
                          ...
                          <Input
                            ...
                            className="bg-slate-900 border-slate-800 text-xs font-mono text-cyan-300"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <Label className="text-xs font-semibold text-slate-300">
                          ...
                          <Input
                            ...
                            className="bg-slate-900 border-slate-800 text-xs font-mono text-slate-300"
                          />
                        </div>
                      </>
                    )}
                    ...
                    {selectedNode.type === "condition" && (
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className="bg-slate-900 border-slate-800 text-xs font-mono text-amber-300"
                        />
                        <p className="text-[10px] text-slate-500">
                        ...
                        {selectedNode.config.channel && (
                          <div className="space-y-1.5">
                            ...
                            <Input
                              ...
                              className="bg-slate-900 border-slate-800 text-xs font-mono text-emerald-400"
                            />
                          </div>
                        )}
                        ...
                        {selectedNode.config.table && (
                          <div className="space-y-1.5">
                            ...
                            <Input
                              ...
                              className="bg-slate-900 border-slate-800 text-xs font-mono text-blue-400"
                            />
                          </div>
                        )}
                      ...
                      <div className="space-y-2 pt-2">
                        {[
                          ...
                          <div
                            ...
                            className="flex items-center justify-between p-2.5 rounded-lg bg-slate-950 border border-slate-800/80 hover:border-cyan-500 cursor-pointer transition-all group"
                          >
                  ...
                  <TabsContent value="test" className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-cyan-300 max-h-80 overflow-y-auto custom-scrollbar">
              ...
              <div className="p-5 border-t border-slate-800 bg-slate-900/80 flex items-center justify-end gap-3">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-semibold shadow-lg shadow-cyan-500/20"
                >
                  Save Configuration
                </Button>
              </div>
      ...
      <Dialog open={isAddStepModalOpen} onOpenChange={setIsAddStepModalOpen}>
        <DialogContent className="bg-slate-950 border-slate-800 text-slate-100 max-w-3xl shadow-2xl backdrop-blur-2xl p-0 overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {group.items.map((item) => (
                    <div
                      ...
                      className="group p-3.5 rounded-xl bg-slate-900/70 border border-slate-800/80 hover:border-cyan-500/80 hover:bg-slate-900 transition-all cursor-pointer shadow-md hover:shadow-cyan-950/30 flex items-start gap-3"
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className="bg-slate-950 border-slate-800 text-slate-400 text-[9px]"
                          >
                            {item.badge}
                          </Badge>
                        </div>
      ...
      <Dialog open={testResultModalOpen} onOpenChange={setTestResultModalOpen}>
        <DialogContent className="bg-slate-950 border-slate-800 text-slate-100 max-w-lg shadow-2xl">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="bg-slate-900/70 p-3 rounded-xl border border-slate-800 space-y-1.5 text-xs text-slate-300 font-mono">
          ...
          <DialogFooter className="pt-3">
            <Button
              ...
              className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-semibold"
            >
